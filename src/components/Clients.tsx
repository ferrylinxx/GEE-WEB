        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8 relative z-10">
          <div className="max-w-3xl">
            ...
            <ScrollReveal delay={300}>
              <p className="mt-8 text-lg md:text-xl text-white/60 leading-relaxed max-w-2xl">{t("heroText")}</p>
            </ScrollReveal>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white py-28">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-8">
            {[0, 1, 2, 3].map((i) => (
              <ScrollReveal key={i} delay={i * 100} variant={i % 2 === 0 ? "fade-up" : "slide-right"}>
                <div className="group relative flex gap-5 p-8 rounded-2xl bg-white border border-gray-100 hover:shadow-xl hover:shadow-[var(--color-accent)]/5 transition-all duration-500 h-full">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-[var(--color-primary)]">{t(`features.${i}.title`)}</h3>
                    <p className="mt-2 text-sm text-[var(--color-text-muted)] leading-relaxed">{t(`features.${i}.desc`)}</p>
                  </div>
                </div>
              </ScrollReveal>
            ))}
